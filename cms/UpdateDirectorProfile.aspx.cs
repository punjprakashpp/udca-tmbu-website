...
public partial class UpdateDirectorProfile : System.Web.UI.Page
{
    ...
    private void LoadPerson()
    {
        ...
        try
        {
            ...
            using (SqlCommand cmd = new SqlCommand(query, con))
            {
                ...
                cmd.Parameters.Add("@Position", SqlDbType.NVarChar).Value = "Director";
    ...
    private void UploadPDF(FileUpload fileUpload, string fileName)
    {
        if (fileUpload.HasFile)
        {
            if (fileUpload.PostedFile.ContentType == "application/pdf")
            {
                try
                {
                    string savePath = Server.MapPath("~/Uploads/Profile/") + fileName;
